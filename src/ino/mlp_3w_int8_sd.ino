...
#define MULT0 1139878597
#define SHIFT0 11
#define MULT1 1456975062
#define SHIFT1 3
#define MULT2 1732617066
#define SHIFT2 7
...
int mlp_predict(int8_t *input) {

    static int32_t buffer1[32];
    static int32_t buffer2[32];

    int32_t *prev = (int32_t*)input;
    int32_t *curr = buffer1;
    ...
    for(int j=0;j<32;j++){
        ...
        // ---- leitura bias ----
        B_files[0].seek(j * sizeof(int32_t));
        B_files[0].read(&acc, sizeof(int32_t));

        // ---- leitura coluna de W ----
        for(int i=0;i<1000;i++){

            int8_t w;
            uint32_t offset = (i*32 + j) * 1;
    ...
    for(int j=0;j<16;j++){
        ...
        // ---- leitura bias ----
        B_files[1].seek(j * sizeof(int32_t));
        B_files[1].read(&acc, sizeof(int32_t));

        // ---- leitura coluna de W ----
        for(int i=0;i<32;i++){

            int8_t w;
            uint32_t offset = (i*16 + j) * 1;
    ...
    for(int j=0;j<3;j++){
        ...
        // ---- leitura bias ----
        B_files[2].seek(j * sizeof(int32_t));
        B_files[2].read(&acc, sizeof(int32_t));

        // ---- leitura coluna de W ----
        for(int i=0;i<16;i++){

            int8_t w;
            uint32_t offset = (i*3 + j) * 1;
...
void setup(){
    ...
    for(int n=0;n<N_TEST;n++){
        ...
        float dt_weight = dt_compute * 0.6; // heur�stica
