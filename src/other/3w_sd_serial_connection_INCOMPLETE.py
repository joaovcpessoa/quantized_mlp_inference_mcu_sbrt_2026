...
def export_arduino_sketch_sd(
    ...
    W_orig = model.model[0].weight.detach().cpu().numpy()  # [32, 1000]
    print("Shape esperado pelo C (out, in):", W_orig.shape)  # deve ser (32, 1000)
    print("Shape atual no W_q[0]:", q['W_q'][0].shape)   # deve ser (1000, 32) ← errado
    # -------- EXPORT BINÁRIOS --------
    for l in range(n_layers):
        q['W_q'][l].T.astype(_NP_SD[bits]).tofile(f"W{l+1}.BIN")
