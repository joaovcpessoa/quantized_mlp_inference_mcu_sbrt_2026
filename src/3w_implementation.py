...
# ════════════════════════════════════════════════════════════════════
# PARTE 1 – TREINO + INFERENCIA EM FP32 NA CPU
# ════════════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════════════
# PARTE 2 – QUANTIZAÇÃO INTEIRA (INT8 / INT16 / INT32)
# ════════════════════════════════════════════════════════════════════
#   bits │ peso (numpy / C) │ bias (numpy / C) │ acumulador MAC
#   ─────┼──────────────────┼──────────────────┼──────────────
#    8   │ int8  / int8_t   │ int32 / int32_t  │ int32_t
#   16   │ int16 / int16_t  │ int32 / int32_t  │ int64_t
#   32   │ int32 / int32_t  │ int64 / int64_t  │ int64_t
#
#   Requantização: multiplicador Q31 (int32) + shift aritmético.
#   Sem ponto flutuante em nenhuma etapa do forward.
...
def get_all_linear_layers(mlp: MLP) -> list[tuple[np.ndarray, np.ndarray]]:
    """
    Return [(W, b), ...] for every Linear layer in mlp.model (in order).
    W has shape [in_features, out_features] transposed from PyTorch's convention
    so that the C MAC loop can be written as  acc += x[i] * W[i*out + j]
    """
    layers = []
...
def forward_numpy(mlp: MLP, x_np: np.ndarray) -> tuple[list[np.ndarray], np.ndarray]:
    """
    Run one forward pass and return (hidden_activations, logits).

    hidden_activations[l] is the post-activation output of hidden layer l
    (after ReLU/Sigmoid/Tanh, before the next linear).  The final output
    layer is returned as `logits` (no activation applied).
    """
    with torch.no_grad():
...
def quantize_multiplier(m: float) -> tuple[int, int]:
    """m ∈ (0,1] → (MULT: int32 Q31, SHIFT: int≥0)
    Representation: m ≈ (MULT / 2^31) * 2^(-SHIFT)
    Application   : result = (acc * MULT) >> (31 + SHIFT)
    """
    if m == 0.0:
        return 0, 0
    shift = 0
